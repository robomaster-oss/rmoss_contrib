    ...
    : TaskImageProc(nh)
{
    ...
    // create pub,sub,srv
    gimbal_ctrl_pub_ = nh_->create_publisher<rm_interfaces::msg::GimbalCmd>("cmd_gimbal", 10);
    shoot_pub_ = nh_->create_publisher<rm_interfaces::msg::ShootCmd>("cmd_shoot", 10);
    robot_state_sub_ = nh_->create_subscription<rm_interfaces::msg::StdRobotState>("robot_state", 10,
        std::bind(&TaskPowerRune::robotStateCallback, this, std::placeholders::_1));
    set_mode_srv_ = nh_->create_service<rm_interfaces::srv::SetMode>("task_power_rune/set_mode",
...
void TaskPowerRune::robotStateCallback(const rm_interfaces::msg::StdRobotState::SharedPtr msg)
{
